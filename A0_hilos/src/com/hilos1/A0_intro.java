...
public class A0_intro extends Thread {

	private static double[] vec = new double[80_000_000]; // 10.000.000

	private int inicio, fin;

	public A0_intro(int inicio, int fin) {
		this.inicio = inicio;
		this.fin = fin;
	}
	...
	// Metodo que NO utiliza el paralelismo y por tanto se ejecuta de forma
	// secuencial
	...
	// Metodo que ejecuta nproc hilos en paralelo y que llaman al metodo run para
	// realizar la multiplicacion del
	...
	private static void vec_concurrente() {
		...
		int inicio = 0, fin = 0;
		A0_intro[] prin = new A0_intro[nproc];
		...
		for (int i = 0; i < nproc; i++) {// Multiplicacion del vector por los nproc hilos
			...
			prin[i] = new A0_intro(inicio, fin);
			prin[i].start();
		}
